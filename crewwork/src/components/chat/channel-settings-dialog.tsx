...
import { Loader2, Hash, Lock, UserPlus, LogOut, Trash2, Circle, X, Crown, Bell, BellOff, Pencil, Check, AlertTriangle, Phone } from 'lucide-react'
...
export function ChannelSettingsDialog({ open, onOpenChange, channel }: ChannelSettingsDialogProps) {
  ...
  // Calls toggle state
  const [callsEnabled, setCallsEnabled] = useState(channel.calls_enabled ?? true)
  ...
  useEffect(() => {
    ...
    setCallsEnabled(channel.calls_enabled ?? true)
    loadMembers()
    loadWorkspaceMembers()
  }, [open, channel.id, channel.calls_enabled])
  ...
  async function handleSaveCallsToggle() {
    const client = getSupabaseClient()
    if (!client) return
    setSavingCalls(true)
    setCallsSaved(false)
    try {
      const { error } = await client
        .from('channels')
        .update({ calls_enabled: callsEnabled })
        .eq('id', channel.id)
      if (error) throw error
      updateChannel(channel.id, { calls_enabled: callsEnabled })
      setCallsSaved(true)
            ...
            {otherMembers.map((m) => (
              <button
                ...
                className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg transition-colors text-left ${
                  transferTarget === m.profile.id ? 'bg-[#FEE2E2]' : 'hover:bg-[#FEF2F2]'
                }`}
                style={{
                  border: transferTarget === m.profile.id ? '1px solid #DC2626' : '1px solid transparent',
                }}
              >
                <div className="relative shrink-0">
                  {m.profile.avatar_url ? (
                    ...
                    <div className="h-7 w-7 rounded-lg flex items-center justify-center text-[11px] font-bold text-white" style={{ background: '#DC2626' }}>
                ...
                {transferTarget === m.profile.id && (
                  <Check className="h-4 w-4 ml-auto" style={{ color: '#DC2626' }} />
                )}
              </button>
            ))}
          </div>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {channel.is_private ? (
              ...
              <Hash className="h-5 w-5" style={{ color: '#DC2626' }} />
            )}
            {isRenaming ? (
              <div className="flex items-center gap-2 flex-1">
                ...
                />
                <Button size="sm" onClick={handleRename} disabled={renaming} className="h-8 px-2" style={{ background: '#DC2626', color: '#fff' }}>
              ...
              <>
                ...
                {isCreator && channel.name !== 'general' && (
                  <button
                    ...
                    className="h-6 w-6 rounded flex items-center justify-center hover:bg-[#FEF2F2] transition-colors"
                    title="Rename channel"
                  >
                    <Pencil className="h-3 w-3" style={{ color: '#A8A29E' }} />
                  </button>
                )}
              </>
            )}
        ...
        {channel.description && (
          <p className="text-[13px]" style={{ color: '#A8A29E' }}>{channel.description}</p>
        )}
        ...
        <div className="space-y-5 pt-2">
          ...
          {/* Voice & Video Calls toggle (admin/creator only, when workspace calls enabled) */}
          {(isAdmin || isCreator) && workspace?.calls_enabled && (
            <div className="space-y-3">
              <Label className="text-[13px] font-semibold uppercase tracking-wider flex items-center gap-2" style={{ color: '#A8A29E' }}>
                <Phone className="h-3.5 w-3.5" />
                Voice & Video Calls
              </Label>
              <div className="p-3 rounded-xl" style={{ background: '#FEF2F2', border: '1px solid #E7E5E4' }}>
                <div className="flex items-center justify-between">
                  <label className="flex items-center gap-2 cursor-pointer">
                    ...
                    />
                    <span className="text-[13px] font-medium" style={{ color: '#1C1917' }}>
                      Enable calls in this channel
                    </span>
                  </label>
                  <Button
                    size="sm"
                    onClick={handleSaveCallsToggle}
          ...
          <div className="space-y-3">
            ...
            {loading ? (
              ...
              <div className="space-y-1">
                {members.map((m) => (
                  <div key={m.profile.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg group hover:bg-[#FEF2F2]">
                    <div className="relative shrink-0">
                      {m.profile.avatar_url ? (
                        <img src={m.profile.avatar_url} alt="" className="h-7 w-7 rounded-lg object-cover" />
                      ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {m.profile.id === user?.id && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: '#FEF2F2', color: '#A8A29E' }}>you</span>
                        )}
                        {m.role === 'admin' && (
          ...
          {isMember && (isAdmin || isCreator) && (
            <div className="space-y-2 pt-2" style={{ borderTop: '1px solid #E7E5E4' }}>
              ...
              {memberSearch && nonMembers.length > 0 && (
                <div className="max-h-32 overflow-y-auto space-y-0.5 rounded-lg border border-[#E7E5E4] p-1">
                  {nonMembers.slice(0, 6).map((p) => (
                    <button
                      key={p.id}
                      onClick={() => handleAddMember(p)}
                      disabled={adding}
                      className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-[#FEF2F2] transition-colors text-left text-[13px]"
                    >
                      <div className="relative shrink-0">
                        {p.avatar_url ? (
                          <img src={p.avatar_url} alt="" className="h-6 w-6 rounded-md object-cover" />
                        ) : (
                          <div className="h-6 w-6 rounded-md flex items-center justify-center text-[10px] font-bold text-white" style={{ background: '#DC2626' }}>
                            {p.display_name[0]?.toUpperCase()}
                          </div>
                        )}
                      </div>
                      <span style={{ color: '#1C1917' }}>{p.display_name}</span>
                      {p.email && <span className="text-[11px] ml-auto" style={{ color: '#A8A29E' }}>{p.email}</span>}
                    </button>
                  ))}
                </div>
              )}
