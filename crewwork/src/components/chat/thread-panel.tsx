...
export function ThreadPanel() {
  const { threadParentMessage, closeThread, user, channels } = useAppStore()
  const [replies, setReplies] = useState<Message[]>([])
  const [loading, setLoading] = useState(true)
  const bottomRef = useRef<HTMLDivElement>(null)
  const repliesRef = useRef<Message[]>([])
  const channelName = channels.find(c => c.id === threadParentMessage?.channel_id)?.name || ''
  ...
  return (
    <div className="w-96 flex flex-col h-full" style={{ background: '#ffffff', borderLeft: '1px solid #E7E5E4' }}>
      {/* Header */}
      <div className="px-5 py-3 flex items-center justify-between shrink-0" style={{ borderBottom: '1px solid #E7E5E4' }}>
        <h3 className="font-bold text-[17px]" style={{ color: '#1C1917' }}>Thread</h3>
        <button className="h-8 w-8 rounded-lg flex items-center justify-center hover:bg-[#FEF2F2] transition-colors" onClick={closeThread}>
          <X className="h-4 w-4" style={{ color: '#A8A29E' }} />
        </button>
      </div>

      {/* Parent message */}
      <div className="px-4 pt-3 pb-2" style={{ borderBottom: '1px solid #E7E5E4' }}>
        <MessageBubble message={threadParentMessage} showHeader isOwn={threadParentMessage.sender_id === user?.id} isThread channelName={channelName} />
      </div>

      {/* Reply count */}
      <div className="px-5 py-2 text-xs font-medium" style={{ color: '#A8A29E', borderBottom: '1px solid #E7E5E4' }}>
