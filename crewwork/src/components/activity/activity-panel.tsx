...
function ActivityItemRow({ item, onNavigate }: { item: ActivityItem; onNavigate: () => void }) {
  ...
  const icon = {
    mention: <AtSign className="h-3.5 w-3.5" style={{ color: '#DC2626' }} />,
    dm: <MessageSquare className="h-3.5 w-3.5" style={{ color: '#DC2626' }} />,
    reaction: <Heart className="h-3.5 w-3.5" style={{ color: '#E55B5B' }} />,
    thread_reply: <MessageSquare className="h-3.5 w-3.5" style={{ color: '#DC2626' }} />,
  }[item.type]
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full text-left px-4 py-3 transition-colors hover:bg-[#FEF2F2] group"
      style={{
        borderBottom: '1px solid #FEF2F2',
        background: !item.read ? '#FEE2E2' : undefined,
        borderLeft: !item.read ? '3px solid #DC2626' : '3px solid transparent',
      }}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1.5 mb-0.5">
            ...
            <span
              className="text-[13px]"
              style={{ color: '#1C1917', fontWeight: !item.read ? 700 : 600 }}
            >
              {senderName}
            </span>
          ...
          <p
            ...
            style={{ color: !item.read ? '#1C1917' : '#78716C' }}
          >
            {contentPreview}
          </p>
...
export function ActivityPanel() {
  ...
  return (
    <div className="w-[380px] flex flex-col h-full" style={{ background: '#fff', borderRight: '1px solid #E7E5E4' }}>
      {/* Header */}
      <div className="px-4 py-3 flex items-center justify-between shrink-0" style={{ borderBottom: '1px solid #E7E5E4' }}>
        <div className="flex items-center gap-2">
          <Bell className="h-4.5 w-4.5" style={{ color: '#DC2626' }} />
          <h3 className="font-bold text-[16px]" style={{ color: '#1C1917' }}>Activity</h3>
        ...
        <div className="flex items-center gap-1">
          {unreadItems.length > 0 && (
            <button
              ...
              className="h-7 px-2 rounded-lg flex items-center gap-1 text-[12px] transition-colors hover:bg-[#FEF2F2]"
              style={{ color: '#DC2626' }}
          ...
          <button
            ...
            className="h-7 w-7 rounded-lg flex items-center justify-center transition-colors hover:bg-[#FEF2F2]"
          >
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {activities.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center px-4">
            ...
            >
              <Bell className="h-7 w-7" style={{ color: '#DC2626' }} />
            </div>
            <h4 className="text-[15px] font-semibold mb-1" style={{ color: '#1C1917' }}>
              No activity yet
            </h4>
          ...
          <>
            ...
            {showUnreadHeader && (
              <div
                ...
                style={{ background: '#FEF2F2', borderBottom: '1px solid #E7E5E4' }}
              >
                <div className="h-1.5 w-1.5 rounded-full" style={{ background: '#DC2626' }} />
                <span className="text-[12px] font-bold uppercase tracking-wide" style={{ color: '#DC2626' }}>
            ...
            {showReadHeader && (
              <div
                ...
                style={{ background: '#FAFAFA', borderBottom: '1px solid #FEF2F2', borderTop: '1px solid #FEF2F2' }}
              >
                <span className="text-[12px] font-semibold uppercase tracking-wide" style={{ color: '#A8A29E' }}>
                  Earlier
                </span>
              </div>
            )}
            ...
            {hasMore && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 text-[13px] font-medium transition-colors hover:bg-[#FEF2F2]"
                style={{ color: '#DC2626', borderTop: '1px solid #FEF2F2' }}
              >
