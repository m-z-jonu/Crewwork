...
export function DmDialog({ open, onOpenChange }: DmDialogProps) {
  const { user, workspace, personalWorkspace, sidebarTab, contacts, setCurrentChannelId, addDmChannel, unhideDm, dmChannels, hiddenDmIds } = useAppStore()
  ...
  useEffect(() => {
    ...
    if (sidebarTab === 'chats') {
      // In Chats mode: use contacts list
      const contactProfiles = contacts
        .map((c) => c.contact_profile)
        .filter((p): p is Profile => !!p && p.id !== user?.id)
      setMembers(contactProfiles)
    } else {
      // In Workspaces mode: use workspace members
      client
  ...
  }, [open, activeWorkspace, sidebarTab, contacts, user])
  ...
  async function startDm(otherUser: Profile) {
    const client = getSupabaseClient()
    if (!client || !user || !activeWorkspace) return
  ...
  async function startGroupDm() {
    const client = getSupabaseClient()
    if (!client || !user || !activeWorkspace || selectedMembers.length < 2) return
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>
            {sidebarTab === 'chats' ? 'Message Contact' : 'Direct Messages'}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="relative">
          ...
          <Input
            placeholder={selectedMembers.length > 0 ? "Add more people..." : sidebarTab === 'chats' ? "Search contacts..." : "Search people..."}
            value={search}
        ...
        <div className="max-h-64 overflow-y-auto space-y-1">
          {filtered.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">
              {search ? 'No members found' : sidebarTab === 'chats' ? 'No contacts yet' : 'No other members in this workspace'}
            </p>
          ) : (
            filtered.map((member) => {
